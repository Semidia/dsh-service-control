...
在 DeepSeek Harness（DSH）Web 侧边栏底部（设置齿轮旁）提供 **「重启服务」** 与 **「关闭服务」** 两个按钮，让你不用回到终端窗口，就能在 UI 里直接重启或关闭整个 `dsh web` 进程。
...
- **自动重启**：重启由一个**完全脱离宿主进程管理**的独立 PowerShell 进程完成（`spawn` detached + unref，宿主退出不会连带终止它），等待 3080 端口释放后重新拉起 `node bin.js web`，日志写入 `restart-web.log(.err)`。
...
- 重启：先 `spawn` 一个 detached 的 `pwsh -EncodedCommand` 重启脚本（含端口轮询与新实例拉起），再 `appExit(0)` 退出宿主。
